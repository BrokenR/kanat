...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//БИТ_Гурьев_ЧТЗ_2.7_20.02.24++
	УстановитьВыборОстаткиСерий();
	КоличествоКОтгрузке = Параметры.КоличествоКОтгрузке;
	//БИТ_Гурьев_ЧТЗ_2.7_20.02.24--
...
//БИТ_Гурьев_ЧТЗ_2.7_21.02.24++
&НаКлиенте
Процедура ОстаткиСерийВыбратьПриИзменении(Элемент)
	
	Если Элементы.ОстаткиСерий.ТекущиеДанные.Выбрать Тогда
		
		Элементы.ОстаткиСерий.ТекущиеДанные.КоличествоУпаковок = Макс(Элементы.ОстаткиСерий.ТекущиеДанные.СвободныйОстаток + Элементы.ОстаткиСерий.ТекущиеДанные.КоличествоУпаковок, 0);
		ИтогоМест = ИтогоМест + 1;
		
	Иначе
		
		Элементы.ОстаткиСерий.ТекущиеДанные.КоличествоУпаковок = 0;
		ИтогоМест = ИтогоМест - 1;
...
КонецПроцедуры //БИТ_Гурьев_ЧТЗ_2.7_21.02.24--
...
//БИТ_Гурьев_ЧТЗ_2.7_20.02.24++
&НаСервере
Процедура УстановитьВыборОстаткиСерий()
	
	Для Каждого Строка Из ОстаткиСерий Цикл
		
		Строка.Выбрать = Строка.КоличествоУпаковок > 0;
		
	КонецЦикла;
	
	ПересчитатьИтогоМест();
	
КонецПроцедуры //БИТ_Гурьев_ЧТЗ_2.7_20.02.24--

//БИТ_Гурьев_ЧТЗ_2.7_21.02.24++
&НаСервере
Процедура ПересчитатьИтогоМест()
